...
- **Monolith Split**: main.rs has been modularized into domain-specific files (handlers, uth, db, pps, media, gent, webhooks, settings, 	emplates, models, security, udit).
...
- **Unified Templates & IPC**: Shared BrandingVars and unified UDS/TCP handler in gent.rs. Introduced ui/static/admin.js for shared CSRF/upload JS logic.
...
- **Bootstrap Admin**: Implemented a random bootstrap admin password (printed once on startup) with forced dmin_must_change_password. The guest account has been removed.
...
- **XSS & Injection Hardening**: Implemented serde_json for app actions (removing ormat! injection risks), escape_html on usernames, and per-request CSP nonces on inline scripts (removing unsafe-inline).
...
## 📝 Audit & Traceability
- **Audit Logging**: Introduced an udit_log table and GET /api/audit route to track container actions, settings, credentials, uploads, webhooks, and users.
...
- **Traceability**: Implemented a container action acknowledgment protocol. The UI now shows real success/error statuses, and WS properly reports gent_connected.
