...
- **Постеры Jellyfin** — настоящие обложки фильмов/эпизодов на карточке стрима, через прокси сервера (API-ключ никогда не попадает в браузер)
- **Управление воспроизведением** — пауза / продолжение / остановка активной сессии Jellyfin (только админы); видно только во время воспроизведения
- **Медиа-интеграции в «Добавить приложение»** — Jellyfin/Emby и Plex настраиваются на карточке приложения; старые данные из Настроек переносятся автоматически
- **Мгновенный статус при перезагрузке** — статусы и CPU/RAM контейнеров отображаются сразу вместо `CHECKING...`
- **Тема Taghawsa** — анимированный WebGL-градиент с эффектом ряби от мыши (всего 38 тем)
- **Исправление действий контейнеров** — `client error (SendRequest)` устранён автоматическим повтором и понятными ошибками
- **v1.8.0** — умный простой, профили производительности, мульти-узел, пакетные интеграции, области API-токенов

Полная история: **[Журнал изменений](https://boubli.github.io/AMUD-Dashboard/docs/changelog)**

### Статус релизов

Рекомендуется: **v1.8.1**. Подробности и отозванные теги: **[английский README](../README.md)** (раздел Release status).
