...
- **Клик по всей карточке** — приложение открывается кликом в любом месте карточки; кнопки и элементы управления внутри работают как раньше
- **Метрики интеграций на виду** — AdGuard, *arr, qBittorrent и др. отображаются на карточке без наведения
- **Привязка диска и сети** — списки интерфейсов и точек монтирования нормализуются и дедуплицируются при сохранении; обновлён текст подсказок в настройках
