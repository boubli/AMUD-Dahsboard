...
- **Умный простой** — опросы и кэши приостанавливаются, когда никто не использует панель; полная скорость при открытии интерфейса (настраиваемый период ожидания)
- **Профили производительности** — Light, Balanced, Active или Custom в Настройки → Производительность и ресурсы
- **Мульти-узловой homelab** — метки приложений и агентов по узлу; телеметрия и контейнеры по хосту
- **Пакетные интеграции** — до 50 карточек в одном API-запросе
- **Области API-токенов** — выбор телеметрии, webhooks, интеграций и др. при создании токена
- **Напоминания о резервной копии** — предупреждение при просроченном экспорте; потоковый экспорт для больших БД
- **Оповещения хоста в простое** — пороги CPU, RAM и диска через webhook при закрытой GUI
- **v1.7.7** — настраиваемые интервалы опроса, переключатель Feeds, светлая тема для каждой темы

Полная история: **[Журнал изменений](https://boubli.github.io/AMUD-Dashboard/docs/changelog)**

### Статус релизов

Рекомендуется: **v1.8.0**. Подробности и отозванные теги: **[английский README](../README.md)** (раздел Release status).
