...
- **الأداء** — فواصل استطلاع قابلة للتكوين؛ وكيل مع sysinfo انتقائي وذاكرة Docker/LXC
- **Feeds** — إيقاف استطلاع RSS في الخلفية وإخفاء صفحة Feeds (المصادر تبقى في قاعدة البيانات)
- **الوضع الفاتح** — لوحات ألوان لكل سمة؛ معاينة الإعدادات تقرأ متغيرات CSS للسمة
- **v1.7.6** — تحسين الذاكرة (تجميع PVE/Docker، mimalloc، Tokio)

السجل الكامل: **[سجل التغييرات](https://boubli.github.io/AMUD-Dashboard/docs/changelog)**

### حالة الإصدار

الإصدار الموصى به: **v1.7.7**. التفاصيل والإصدارات المسحوبة: **[README الإنجليزي](../README.md)** (قسم Release status).
...
على عكس لوحات التحكم القديمة (Heimdall، Homepage، Homarr) التي تعمل على بيئات تشغيل ثقيلة (PHP-FPM، Node.js) وتعتمد على ملفات تكوين YAML المعقدة والمتداخلة، فإن AMUD مكتوب بلغة Rust المجمعة ويتم حفظ البيانات بالكامل في SQLite. وبشكل عام، يستهلك الخادم ووكيل القياس عن بعد ما بين **35 ميجابايت إلى 100 ميجابايت من ذاكرة الوصول العشوائي (RAM)** في وضع الخمول مع وقت تنفيذ للمسارات أقل من جزء من الملي ثانية.
...
| **حجم الذاكرة في وضع الخمول** | ~150 ميجابايت | **35 ميجابايت - 100 ميجابايت** (مشترك) |
