...
- **ملصقات Jellyfin** — صور حقيقية للأفلام/الحلقات على بطاقة البث، عبر وكيل الخادم (مفتاح API لا يصل أبداً إلى المتصفح)
- **أزرار التحكم بالتشغيل** — إيقاف مؤقت / استئناف / إيقاف لجلسة Jellyfin النشطة (للمشرفين فقط)؛ تظهر فقط أثناء التشغيل
- **تكاملات الوسائط في «إضافة تطبيق»** — يتم إعداد Jellyfin/Emby وPlex على بطاقة التطبيق؛ بيانات الاعتماد القديمة من الإعدادات تُنقل تلقائياً
- **حالة فورية عند إعادة التحميل** — تظهر الحالات وCPU/RAM للحاويات فوراً بدلاً من `CHECKING...`
- **سمة Taghawsa** — تدرج متحرك WebGL مع تأثير تموج الفأرة (38 سمة إجمالاً)
- **إصلاح إجراءات الحاويات** — حُل خطأ `client error (SendRequest)` بإعادة محاولة تلقائية ورسائل خطأ واضحة
- **v1.8.0** — الخمول الذكي، ملفات الأداء، تعدد العقد، التكاملات الدفعية، نطاقات رموز API

السجل الكامل: **[سجل التغييرات](https://boubli.github.io/AMUD-Dashboard/docs/changelog)**

### حالة الإصدار

الإصدار الموصى به: **v1.8.1**. التفاصيل والإصدارات المسحوبة: **[README الإنجليزي](../README.md)** (قسم Release status).
