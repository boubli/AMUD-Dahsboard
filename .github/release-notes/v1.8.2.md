...
### Bugs introduced (this release — fixed in later versions)

- **Desktop layout broken on Windows** — stray `}` in `.dashboard-container` orphaned `gap` and broke full-width flex layout (fixed in v1.8.3/v1.8.4)
- **Drag reorder wrong placement** — Y-only hit test and live DOM shuffle forced cards into wrong grid slots (fixed in v1.8.4)
- **Drag visual artifacts** — orange glow lines and stuck `drag-over` state on cards (fixed in v1.8.4)

### Known issues (at release time)

- Greeting shimmer could make username invisible on some Chromium builds (addressed in v1.8.3)
