...
- **In-app updater on Proxmox** — `SHA256SUMS` entries are matched by filename (`ui.tar.gz`, `amud-server`, etc.) instead of brittle full-path strings, so checksum verification works again.
- Release workflow generates `SHA256SUMS` with the same basename layout the updater expects.
