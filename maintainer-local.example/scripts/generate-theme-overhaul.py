...
# Extra :root variables merged into the single :root block (avoids duplicate selectors).
PROFILE_ROOT_EXTRAS: dict[str, list[str]] = {
    "crt_green": [
        "    --radius-xl: 4px;",
    ...
    "matrix": [
        "    --theme-font: 'Courier Prime', monospace;",
    ],
    "arcade": [
        "    --radius-xl: 0;",
...
def load_old_manifest() -> dict:
    with open(UI_THEMES / MANIFEST_JSON, encoding="utf-8") as f:
        return json.load(f)
...
def sync_docs() -> None:
    ...
    shutil.copy2(UI_THEMES / MANIFEST_JSON, DOCS_THEMES / MANIFEST_JSON)
...
def main() -> None:
    ...
    (UI_THEMES / MANIFEST_JSON).write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
    sync_docs()
    profile_count = len({s["profile"] for s in THEME_SPECS.values()})
    print(f"Generated {len(THEME_SPECS)} CSS files, {profile_count} layouts, icon packs, manifest v5")
